...
const DEFAULT_TABLES = {
  ...
  "20_Attendance_Devices": [
    { device_id: "MCC-VPSG-TRET", device_name: "MCC TANG TRET", name: "MCC TANG TRET", ip: "113.161.53.133", port: 5007, serial: "AYSH02091522", location: "VPSG", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công Tầng Trệt VPSG" },
    { device_id: "MCC-VPSG-T3", device_name: "MCC T3", name: "MCC T3", ip: "113.161.53.133", port: 5006, serial: "AYSH02091575", location: "vpsg", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công Tầng 3 VPSG" },
    { device_id: "MCC-VPSG-T2", device_name: "MCC T2", name: "MCC T2", ip: "113.161.53.133", port: 5005, serial: "AYSH02091571", location: "vpsg", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công Tầng 2 VPSG" },
    { device_id: "MCC-TLMT-TP", device_name: "TL-MT TP", name: "TL-MT TP", ip: "113.161.201.71", port: 5005, serial: "AYSB28014633", location: "HCM-TLMT", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công TL-MT TP.HCM" },
    { device_id: "MCC-TLMT-TH", device_name: "TL-MT TH", name: "TL-MT TH", ip: "14.224.132.5", port: 5005, serial: "ZXRC17014917", location: "TL-MT TH", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công TL-MT Long An" },
    { device_id: "MCC-NUI-VUNG", device_name: "NUI VUNG", name: "NUI VUNG", ip: "113.161.194.20", port: 5005, serial: "AYSH02091656", location: "NUI VUNG", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công Núi Vung" },
    { device_id: "MCC-KHBMT-VP", device_name: "KH-BMT VP", name: "KH-BMT VP", ip: "113.161.30.79", port: 5006, serial: "ZXRC17014860", location: "KH-BMT", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công KH-BMT Văn Phòng" },
    { device_id: "MCC-KHBMT-HAM", device_name: "KH-BMT HẦM", name: "KH-BMT HẦM", ip: "14.224.151.151", port: 5005, serial: "ZXRC17014867", location: "KHBMT", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công KH-BMT Hầm" },
    { device_id: "MCC-KHBMT-KHUD", device_name: "KH-BMT KHU D", name: "KH-BMT KHU D", ip: "113.161.30.79", port: 5005, serial: "AYSB28014684", location: "KHBMT", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công KH-BMT Khu D" },
    { device_id: "MCC-CTVP-VP", device_name: "CTVP VP", name: "CTVP VP", ip: "117.2.32.120", port: 5005, serial: "ZXRC17014844", location: "CTVP", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công CTVP Văn Phòng" },
    { device_id: "MCC-CTVP-DUAN", device_name: "CTVP DU AN", name: "CTVP DU AN", ip: "117.2.32.120", port: 5006, serial: "ZXRC17014905", location: "CTVP", in_out_mode: "AUTO", enabled: true, status: "ONLINE", note: "Máy Chấm Công CTVP Dự Án" }
  ]
};
...
function normalizeDepartmentName(codeOrName) {
  ...
  const low = s.toLowerCase();
  // Trung Hải
  if (low.includes('tổng giám đốc') || low === 'bgd' || low === 'btgd' || low === 'btgd.th' || low === 'bgd.th') return 'BAN TỔNG GIÁM ĐỐC TRUNG HẢI';
  if (low.includes('tổ chức hành chính') || low === 'btchc' || low === 'btchc.th' || low === 'b.tchc' || low === 'tchc.th') return 'BAN TỔ CHỨC HÀNH CHÍNH TRUNG HẢI';
  if ((low.includes('tài chính kế toán') && (low.includes('trung hải') || low.includes('.th'))) || low === 'btckt' || low === 'btckt.th' || low === 'tckt.th') return 'BAN TÀI CHÍNH KẾ TOÁN TRUNG HẢI';
  if ((low.includes('kế hoạch tổng hợp') && (low.includes('trung hải') || low.includes('.th'))) || low === 'bkhth' || low === 'bkhth.th' || low === 'khth.th' || low === 'pkhtk.th') return 'BAN KẾ HOẠCH TỔNG HỢP TRUNG HẢI';
  if (low.includes('pháp chế') || low === 'bpc' || low === 'bpc.th' || low === 'pc.th') return 'BAN PHÁP CHẾ TRUNG HẢI';
  if (low.includes('thiết bị') || low === 'bqltb' || low === 'bqltb.th' || low === 'qltb.th' || low === 'ptb.th') return 'BAN QUẢN LÝ THIẾT BỊ TRUNG HẢI';
  if ((low.includes('trực tiếp') && (low.includes('trung hải') || low.includes('.th'))) || low === 'tructiep_bdhda.th' || low === 'tructiep.th') return 'KHỐI TRỰC TIẾP-DỰ ÁN TRUNG HẢI';
  if ((low.includes('gián tiếp') && (low.includes('trung hải') || low.includes('.th'))) || low === 'giantiep_bdhda.th' || low === 'giantiep.th') return 'KHỐI GIÁN TIẾP- DỰ ÁN TRUNG HẢI';
  if ((low.includes('văn phòng') && (low.includes('trung hải') || low.includes('.th'))) || low === 'vp_bdhda.th' || low === 'vp.th') return 'KHỐI VĂN PHÒNG-DỰ ÁN TRUNG HẢI';
  ...
  // Phú Minh
  if (low.includes('phú minh') || low.includes('.pm')) {
    if (low.includes('giám đốc') || low === 'bgd.pm' || low === 'btgd.pm') return 'BAN GIÁM ĐỐC PHÚ MINH';
    if (low.includes('điều hành dự án') || low === 'bdhda.pm') return 'BAN ĐIỀU HÀNH DỰ ÁN PHÚ MINH';
    if (low.includes('hành chính nhân sự') || low === 'phcns.pm' || low === 'hcns.pm') return 'PHÒNG HÀNH CHÍNH NHÂN SỰ PHÚ MINH';
    if (low.includes('kế hoạch') || low === 'pkhth.pm' || low === 'khth.pm') return 'PHÒNG KẾ HOẠCH TỔNG HỢP PHÚ MINH';
    if (low.includes('kế toán') || low === 'ptckt.pm' || low === 'tckt.pm') return 'PHÒNG TÀI CHÍNH KẾ TOÁN PHÚ MINH';
    if (low.includes('trực tiếp') || low.includes('tructiep') || low === 'tructiep_bdhda.pm') return 'KHỐI TRỰC TIẾP-DỰ ÁN PHÚ MINH';
    if (low.includes('gián tiếp') || low.includes('giantiep') || low === 'giantiep_bdhda.pm') return 'KHỐI GIÁN TIẾP_DỰ ÁN PHÚ MINH';
  }

  // Thành Phát
  if (low.includes('thành phát') || low.includes('.tp')) {
    if (low.includes('giám đốc') || low === 'bgd.tp' || low === 'btgd.tp') return 'BAN GIÁM ĐỐC THÀNH PHÁT';
    if (low.includes('hành chính nhân sự') || low === 'phcns.tp' || low === 'hcns.tp') return 'PHÒNG HÀNH CHÍNH NHÂN SỰ THÀNH PHÁT';
    if (low.includes('kế hoạch') || low === 'pkhth.tp' || low === 'khth.tp') return 'PHÒNG KẾ HOẠCH TỔNG HỢP THÀNH PHÁT';
    if (low.includes('kế toán') || low === 'ptckt.tp' || low === 'tckt.tp') return 'PHÒNG TÀI CHÍNH KẾ TOÁN THÀNH PHÁT';
    if (low.includes('trực tiếp') || low.includes('tructiep') || low === 'tructiep_bdhda.tp') return 'KHỐI TRỰC TIẾP-DỰ ÁN THÀNH PHÁT';
    if (low.includes('gián tiếp') || low.includes('giantiep') || low === 'giantiep_bdhda.tp') return 'KHỐI GIÁN TIẾP-DỰ ÁN THÀNH PHÁT';
  }

  // Trung Nam
  if (low.includes('trung nam') || low.includes('.tn')) {
    if (low.includes('giám đốc') || low === 'bgd.tn' || low === 'btgd.tn') return 'BAN GIÁM ĐỐC TRUNG NAM';
    if (low.includes('hành chính nhân sự') || low === 'phcns.tn' || low === 'hcns.tn') return 'PHÒNG HÀNH CHÍNH NHÂN SỰ TRUNG NAM';
    if (low.includes('thương mại') || low.includes('kinh doanh') || low === 'pkdtm.tn' || low === 'kdtm.tn') return 'PHÒNG KINH DOANH THƯƠNG MẠI TRUNG NAM';
    if (low.includes('kế toán') || low === 'ptckt.tn' || low === 'tckt.tn') return 'PHÒNG TÀI CHÍNH KẾ TOÁN TRUNG NAM';
  ...
  try {
    ...
    if (path === "contracts" || path.startsWith("contracts/")) {
      ...
      if (method === "GET") {
        ...
        // Enrich all contracts with latest employee info
      ...
      if (contractId && subAction === "appendices" && method === "POST") {
        ...
        // If appendix adjusts salary or extension, update base contract
      ...
      if (contractId && subAction === "terminate" && method === "POST") {
        ...
        // Optional: Update employee status to 'Đã nghỉ việc'
    ...
    if (path.startsWith("attendance/")) {
      ...
      if (path === "attendance/calculate" && method === "POST") {
        ...
        // Map all shifts and schedules dynamically
        const shiftsList = data.tables["15_Attendance_Shifts"] || [];
        const shiftMap = {};
        ...
        (data.tables["16_Attendance_Schedules"] || []).forEach(sc => {
          const d = String(sc.department_name || sc.department_id || '').toLowerCase().trim();
          if (d && sc.shift_id) deptScheduleMap[d] = sc.shift_id;
        });

        employees.forEach(emp => {
          const empCode = String(emp.attendance_code || emp.time_attendance_code || '').trim();
          const deptClean = String(emp.department_name || emp.department_id || '').toLowerCase().trim();
          ...
          sortedDates.forEach(dt => {
            ...
            // Cơ chế 1: Tự động nhận diện ca thông minh
      ...
      if (path === "attendance/zk/sync" && method === "POST") {
        ...
        if (Array.isArray(punches) && punches.length > 0) {
          ...
          punches.forEach(p => {
            const key = `${p.attendance_code}_${p.timestamp}`;
            if (!existingKeys.has(key)) {
              logs.unshift({
                ...
                verify_type: p.verify_type || 'Khuôn mặt',
                device_name: p.device_name || 'Máy Chấm Công',
        ...
        return jsonResponse({
          success: true,
          message: `Đã đồng bộ thành công ${addedCount} bản ghi chấm công từ máy chấm công!`,
